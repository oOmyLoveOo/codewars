...
// First we evaluate if it's a prime number
const isPrime = (num) => {
    if(num < 2) return false;
    for(let i = 2; i <= Math.sqrt(num); i++){
        if(num % i === 0) return false;  // Check divisibility by i
    }
    return true;
};
...
function gap(g, m, n) {
    let primes = [];
    // Add the prime numbers in the array primes
    for(let i = m; i <= n; i++){
        if (isPrime(i)) primes.push(i);
    }

    // Look for the first pair of primes with gap g
    for(let i = 0; i < primes.length - 1; i++){
        if(primes[i + 1] - primes[i] === g){
            return [primes[i], primes[i + 1]];  // Return the first pair found
        }
    }
